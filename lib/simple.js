    ...
    let sock = Object.defineProperties(conn, {
        ...
        sendMedia: {
            async value(jid, path, quoted, options = {}) {
                 ...
                 return await conn.sendMessage(jid, { [`${pase}`]: data, mimetype: mime, ...options }, { quoted, ...options })
            }
        },
        sendSticker: {
            async value(jid, path, quoted, options = {}) {
                 ...
                 return await conn.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
            }
        },
        ...
        sendContact: {
             async value(jid, number, name, quoted, options)  {
...
END:VCARD`.trim()
                ...
                return await conn.sendMessage(jid, {
                    ...
                    contacts: {
                        ...options,
                        displayName: (contacts.length >= 2 ? `${contacts.length} 2003` : contacts[0].displayName) || null,
                        contacts,
                    }
                }, { quoted, ...options })
            },
        ...
        fakeReply: {
            ...
            value(jid, text = '', fakeJid = this.user.jid, fakeText = '', fakeGroupJid, options) {
                return conn.sendMessage(jid, { text: text, mentions: conn.parseMention(text) }, { quoted: { key: { fromMe: areJidsSameUser(fakeJid, conn.user.id), participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options } })
            },
            writable: true,
        }, 
        react: {
        ...
        sendListM: {
            async value(jid, button, rows, quoted, options = {}) {
                 const sections = [
                     {
                       title: button.titleRow || button.titleRows || '',
                       rows: [...rows]
                     }
                 ]
                 const listMessage = {
                     ...options,
                     title: button.title || button.titleText || '',
                     text: button.text || button.textText || 'Hello',
                     footer: button.footer || button.footerText || '',
                     mentions: conn.parseMention(button.text + button.textText + button.title + button.titleText + button.footer + button.footerText + button.button + button.buttonText),
                     buttonText: button.button || button.buttonText|| 'Click Here',
                     sections
                 }
